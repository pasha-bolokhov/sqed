...
#define RAD    2.5
  ...
  xy              arcpt1(RAD, 0);    arcpt1.rotate(-135); arcpt1 += xy(0,RAD);
  xy              arcpt2(RAD, 0);    arcpt2.rotate(135); arcpt2 += xy(0,RAD);
  xy              arcpt3(RAD, 0);    arcpt3 += xy(0,RAD);

  arcpt1 += xy(0, SB_GAUGE_LIFT1);
  arcpt2 += xy(0, SB_GAUGE_LIFT1);
  arcpt3 += xy(0, SB_GAUGE_LIFT1);
  ...
  xy                     lv_coord(-RAD, RAD + SB_GAUGE_LIFT1);
  vertex_circlecross     lv(fd, lv_coord);        /* the LV vertex */

  vertex_dot             v1(fd, 0, 
			    SB_GAUGE_LIFT1);  /* the ordinary SQED vertex */
  v1.radius.scale(1.7);
  ...
  line_plain             f2(fd, lv, sb);
  line_plain             f3(fd, sb, v1);
